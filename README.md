...
| 文件 | 内容 | 迁移部署时 |
|------|------|-----------|
| [`config/.env`](config/.env.example) | 数据库、密钥、API Key、推送兜底、时区、日志 | **每个环境各配一份** |
...
| 推送行为 | 相关通道**自动停用**，不会回落到 `.env` 里的共享凭据 |
| 登录状态 | cookie 失效，所有人需要重新登录 |

最后一条是关键：如果解密失败被当成「该用户没配过」，推送就会沿用 `.env` 里的兜底凭据，把这个用户的提醒发到共享邮箱去。所以这里宁可停用也不猜。
...
推送凭据从全局 `config/.env` 迁移到了**每个用户自己的「推送设置」页**：SMTP 密码与 WPUSH APIKEY 加密存库，页面上只回显掩码。`config/.env` 中的 `SMTP_*` / `WPUSH_*` 仍作为**兜底**——用户没填写时沿用这套共享配置，所以升级后原有推送不会中断。
